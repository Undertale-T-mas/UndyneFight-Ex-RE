...
namespace Rhythm_Recall.Waves
{
    public class SuddenChange : IChampionShip
    {
        ...
        public class Game : WaveConstructor, IWaveSet
        {
            ...
            {
                public override string BarrageAuthor => GameStates.difficulty != 5 ? "T-mas" : "TK";
                public override string PaintAuthor => "Kaeny";
                public override string SongAuthor => "Xylium";
                public override string DisplayName => "Bullet Hell";
            ...
            public void ExtremePlus()
            {
                ...
                {
                    if (At0thBeat(16))
                        for (int i = 0; i < 20; i++) CreateBone(new SideCircleBone(i * 18, 3, (i % 4) == 0 ? 115 : 35, BeatTime(7.3f)));
                    if (AtKthBeat(16, BeatTime(3.9f)))
                        CreateEntity(new GunBullet(Heart.Centre, BeatTime(4), new float[] { 0, 90, 270, 180 }));
                    if (AtKthBeat(16, BeatTime(7.2f)))
                        CreateEntity(new GunBullet(Heart.Centre, BeatTime(4), new float[] { 45, 135, 225, 315 }));
                    if (AtKthBeat(16, BeatTime(10f)))
                    {
                        float[] dirs = new float[4];
                        for (int i = 0; i < 2; i++) { dirs[i] = Rand(0, 360); dirs[i + 2] = LastRand + 180; };
                        CreateEntity(new GunBullet(Heart.Centre, BeatTime(4), dirs));
                    }
                }
                ...
                if (InBeat(256 + 2, 256 + 127) && GametimeF % 6 == 0)
                {
                    ...
                    float downPos = Sin(GametimeF * 3) * -39f - 11f;
                ...
                if (InBeat(256, 384 + 127) && AtKthBeat(16, 0))
                {
                    ...
                    if (InBeat(256, 368))
                        CreateEntity(new GunBullet(Heart.Centre, BeatTime(4), new float[] { 90, 270 }));
                }
                ...
                if (InBeat(384))
                {
                    CreatePlatform(new Platform(1, new Vector2(320, 310), Motions.PositionRoute.YAxisSin, Motions.LengthRoute.autoFold, Motions.RotationRoute.sin)
                    {
                        PositionRouteParam = new float[] { 0, 80, BeatTime(32), BeatTime(8) },
                        LengthRouteParam = new float[] { 40, BeatTime(124) },
                        RotationRouteParam = new float[] { 32, BeatTime(16), 0, 0 }
                    });
                    CreatePlatform(new Platform(0, new Vector2(320, 310), Motions.PositionRoute.YAxisSin, Motions.LengthRoute.autoFold, Motions.RotationRoute.sin)
                    {
                        PositionRouteParam = new float[] { 0, 80, BeatTime(32), BeatTime(24) },
                        LengthRouteParam = new float[] { 40, BeatTime(124) },
                        RotationRouteParam = new float[] { 32, BeatTime(16), BeatTime(8), 0 }
                    });
                ...
                if (InBeat(512, 536) && At0thBeat(8))
                {
                    CreateGB(new NormalGB(Heart.Centre + GetVector2(150, Rand(0, 359)), Heart.Centre, new Vector2(1.0f, 1.0f), BeatTime(8), 10));
                }
                if (InBeat(540, 568) && At0thBeat(4))
                {
                    CreateGB(new NormalGB(Heart.Centre + GetVector2(150, Rand(0, 359)), Heart.Centre, new Vector2(1.0f, 0.5f), BeatTime(8), 10));
                    CreateGB(new NormalGB(Heart.Centre + GetVector2(150, Rand(0, 359)), Heart.Centre, new Vector2(1.0f, 0.5f), BeatTime(8), 10));
                }
                if (InBeat(256))
                {
                ...
                if (InBeat(576))
                {
                    ...
                    for (int i = 0; i < 17; i++)
                    {
                        CreateBone(new CustomBone(new Vector2(210, 200 + i * 10), Motions.PositionRoute.cameFromLeft, Motions.LengthRoute.sin, Motions.RotationRoute.stableValue)
                        {
                            ...
                            LengthRouteParam = new float[] { 60, BeatTime(16), BeatTime(i), 120 }
                        });
                        CreateBone(new CustomBone(new Vector2(430, 200 + i * 10), Motions.PositionRoute.cameFromRight, Motions.LengthRoute.sin, Motions.RotationRoute.stableValue)
                        {
                            RotationRouteParam = new float[] { 90 },
                            LengthRouteParam = new float[] { 60, BeatTime(16), BeatTime(i + 8), 120 }
                        });
                    }
                }
                if (InBeat(700))
                {
                    for (int i = 0; i < 44; i++)
                    {
                        CreateBone(new CustomBone(new Vector2(100 + i * 10, 360), Motions.PositionRoute.cameFromDown, Motions.LengthRoute.sin, Motions.RotationRoute.stableValue)
                        {
                            RotationRouteParam = new float[] { 0 },
                            LengthRouteParam = new float[] { 20, BeatTime(44), BeatTime(i), 50 }
                        });
                        CreateBone(new CustomBone(new Vector2(100 + i * 10, 200), Motions.PositionRoute.cameFromUp, Motions.LengthRoute.sin, Motions.RotationRoute.stableValue)
                        {
                            RotationRouteParam = new float[] { 0 },
                            LengthRouteParam = new float[] { 20, BeatTime(44), BeatTime(i + 22), 50 }
                        });
                    }
                    SetSoul(3);
                }
